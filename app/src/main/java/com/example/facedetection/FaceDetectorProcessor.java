...
public class FaceDetectorProcessor extends VisionProcessorBase<List<Face>> {
    ...
    private onFaceDetectListener nOnFaceDetectListener;

    public FaceDetectorProcessor(Context context,onFaceDetectListener nOnFaceDetectListener) {
        super(context);
        this.nOnFaceDetectListener = nOnFaceDetectListener;
    ...
    protected void onSuccess(@NonNull List<Face> faces, @NonNull GraphicOverlay graphicOverlay) {
        if(faces.size()==0){
            nOnFaceDetectListener.onFaceDetector(false);
        }
        else{
            nOnFaceDetectListener.onFaceDetector(true);
        }
        for (Face face : faces) {
            graphicOverlay.add(new FaceGraphic(graphicOverlay, face,nOnFaceDetectListener));
